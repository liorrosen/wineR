...
# wine_cellar1 <- readRDS("wine_collection_2016WHITE.RDS")
# wine_cellar2 <- readRDS("wine_collection_2016REDUS.RDS")
# wine_cellar3 <- readRDS("wine_collection_V2.RDS")
# wine_cellar <- c(wine_cellar1,wine_cellar2,wine_cellar3)
...
#term_matrix <- term_matrix %>% t()
#wine_terms <- wine_terms[colSums(term_matrix)>5]
#term_matrix <- term_matrix[,colSums(term_matrix)>5] %>% t()
...
Now we have a model of wine reviews: ~22000 wine reviews, possessing a combination of ~300 features (where each term is a feature). Given this model, we'll use principal components analysis (PCA) to see if there are a few features that help separate wines from one another, with these features being principle components (PCs) of the PCA composed of weighted combinations of wine terms.
...
##TESTICA
#library(fastICA)
#ica_results<-fastICA(t(term_matrix),2,row.norm = TRUE)
#pcs<-cbind(ica_results$S,wine_all.new)
#colnames(pcs)=c("PC1", "PC2", colnames(pcs[3:ncol(pcs)]))
...
plt1 <- ggplot(subset(pcs,Variety %in% c("White Blend","Rosé", "Red Blend")), 
             aes(x = PC1, y = PC2, colour = Variety)) + geom_point( alpha = 1/2) + xlim(-6, 9) + ylim(-7, 8) + 
...
hist1<-ggplot(subset(pcs,Variety %in% c("White Blend","Rosé", "Red Blend")), 
             aes(x = PC1, fill = Variety))+geom_density(binwidth=0.1,alpha = 1)+theme(legend.position="none")+ xlim(-6, 9)

hist2<-ggplot(subset(pcs,Variety %in% c("Chardonnay",  "Sauvignon Blanc", "Syrah","Cabernet Sauvignon","Sangiovese")), 
             aes(x = PC1, fill = Variety))+geom_density(binwidth=0.1,alpha = 1)+theme(legend.position="none")+ xlim(-6, 9)
...
pc_wineload = aggregate(pcs[1:2], by= list(pcs$Variety), FUN = mean)
...
num_pcs = 20
mldata = cbind(pcs2[,1:num_pcs],pcs2[,274])
...
image = qplot(x = true, y = predicted, fill=value, data=image_data,geom="raster")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5))+scale_fill_continuous(name = "classification %:")+geom_text(label=round(image_data$value), color = "white")
plot(image)
  ...
  for (numpcs in npcs){
    ...
    for (niter in c(1:niter)){
      ...
      if (group2fit == "Variety"){
        mldata <- cbind(pcs2[,1:numpcs],pcs2[,274])
        colnames(mldata)[numpcs+1] <- "Target"
      }
      
      if (group2fit == "Category"){
        mldata <- cbind(pcs2[,1:numpcs],pcs2[,275])
        colnames(mldata)[numpcs+1] <- "Target"
      }
...
pcs2 <- pcs %>% group_by(Variety) %>% filter(n() >= 800) %>% ungroup() %>% as.data.frame()
pcs2 <- pcs2 %>% filter(!grepl("Blend",pcs2$Variety, fixed=TRUE))
group2fit = "Variety"
...
pcs2 <- pcs %>% group_by(Variety) %>% filter(n() >= 250) %>% ungroup() %>% as.data.frame()
pcs2 <- pcs2 %>% filter(!grepl("Blend",pcs2$Variety, fixed=TRUE))
group2fit = "Variety"
