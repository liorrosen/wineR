...
```{r pca_calc_and_projection, include=TRUE, results='hide', fig.height=8, fig.width=8}
options(warn=-1)
...
```{r pca_loadings_plot, include=TRUE, results='hide', fig.height=5, fig.width=6}

textplt1 <- ggplot(as.data.frame(pc_results$x), aes(x=PC1, y=PC2, label = wine_terms))  +geom_label(label.size =0.1 ,	label.padding = unit(0.11,"lines"), fontface = "bold", size = 3) +
...
```{r classification_trial, include=TRUE, results='hide', fig.height=5, fig.width=7}
...
```{r classification_how_many_PCs2, include=TRUE, results='hide'}
...
```{r classification_how_many_PCs3, include=TRUE, results='hide'}
