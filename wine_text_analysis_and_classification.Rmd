...
hist1<-ggplot(subset(pcs,Variety %in% c("White Blend","Rosé", "Red Blend")), 
             aes(x = PC1, fill = Variety))+geom_density(binwidth=0.1,alpha = .8)+theme(legend.position="none")+ xlim(-6, 9)
...
textplt1 <- ggplot(as.data.frame(pc_results$rotation), aes(x=PC1, y=PC2, label = wine_terms)) + geom_text(check_overlap=TRUE,fontface = "bold", size = 4)
...
Ok so this makes sense, PC1 seems to call out a number of big red and white wine terms, as tannins, cherry, spice and plum flavour all play a big role in wines ending on the negative end of PC1, and apple, lime, peach, and citrus, traditionally white wine terms, play a role in wines falling on the positive side of PC1. PC2 doesn't lend itself to nearly as simple an interpretation, but it, along with the next few dimensions add substantially to the abiliyt to classify individual wine varieties from one another.
...
They look like they classify pretty well: on average 80% of reviews were correctly assigned to the variety they belonged to, and if we're misclassifying it's within the same category.  While it's not suprising we can tell apart reds from whites pretty well (they're usually described with different terms, after all), it's reassuring we're quite good at telling wines apart within red and white categories. 
