...
plt4 <- ggplot(subset(pcs,Category %in% c("Dessert", "Sparkling","Port/Sherry")), 
               aes(x = PC1, y = PC2, colour = Category)) + geom_point( alpha = 1/2) + xlim(-6, 9) + ylim(-7, 8) +
  guides(col = guide_legend(nrow = 2))+ theme(legend.position = "bottom")
...
Ok so this makes sense, PC1 seems to call out a number of big red and white wine terms, as tannins, cherry, spice and plum flavour all play a big role in wines ending on the positive end of PC1, and apple, pear, peach, and citrus, traditionally white wine terms, play a role in wines falling on the negative side of PC1.  What's funny is that a single feature plays a strong role on wines falling along PC2 - and that's if the wine was described as "ripe", and secondarily, which I infer to be a signal that a wine is particularly "fruity", though I need to dig a bit deeper into how people use the term technically. 
...
mldata = cbind(pcs2[,1:150],pcs2[,274])
...
predconfmat$result <- t(apply(predconfmat$result[,1:5], 1, function(x) 100*(x/sum(x))))
