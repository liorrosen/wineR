...
Now that we have our simple model of wine reviews, let's try and simplify it through principal components analysis (PCA) to see if there are a few dimensions that help seperate wines into groups by category or varietal
...
Ok so this makes sense - PC1 seesm to call out a number of big red wine terms - tannins, and cherry, spice and rasberry flavor all play a big role in PC1, while PC2 seems to be loaded with many traditionally white-wine terms - apple, lemon, pear, peach, and ripe fruits all fall heavily on the white wine (-) side of this axis - while tobacco, tannins, cherry are all on the red (+) side of this axis, too.  

so if we have all this information to tell apart wines how well do we really do at classifying wines by category or varietal from these terms?  Let's do a quick classification of just the "Big" wines -- wines varieties that have more than 300 reviews in our database - can we tell them apart?  
...
```{r classification_evaluation, include=TRUE, echo = FALSE, results='hide'}
...
it seems like both support vector machines (SVM) and randomForests (rf) do the trick - they're the best at classifying most datasets - though are they efficient? 
...
though, for this data the svms are a fair bit quicker to train - so let's use those going foward. 

```{r classification_how_many_PCs, include=TRUE, echo = FALSE, results='hide'}
...
```
If we were using a standard "elbow method" to evaluate the dimensionality of this data - perhaps we'd take 12, 25 or 50 PCs on the basis that adding more gave diminising returns that said

```{r classification_how_many_PCs2, include=TRUE, echo = FALSE, results='hide'}
