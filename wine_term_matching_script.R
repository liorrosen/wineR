...
plt1 <- ggplot(subset(pcs,Variety %in% c("White Blend","Rosé", "Red Blend")), 
               aes(x = PC1, y = PC2, colour = Variety)) + geom_point( alpha = 1/2) + guides(col = guide_legend(nrow = 2)) + theme(legend.position = "bottom")+ xlim(-.004, .044) + ylim(-.041, .025)
...
plt4 <- ggplot(subset(pcs,Category %in% c("Dessert", "Sparkling","Port/Sherry")), 
               aes(x = PC1, y = PC2, colour = Category)) + geom_point( alpha = 1/2)  + guides(col = guide_legend(nrow = 2))+ theme(legend.position = "bottom") + xlim(-.004, .044) + ylim(-.041, .025)
